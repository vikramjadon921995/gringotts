  ...
  describe "void" do
    test "successful response of void with right params", %{opts: opts} do
      use_cassette "sagepay/successful response of void with right params" do
        opts_authoriize = opts[:opts_authoriize] ++ [transaction_type: "Payment"]
        {:ok, response} = SagePay.authorize(@amount, @card, opts_authoriize)
        assert {:ok, response} = SagePay.void(response.id, opts_authoriize)
      end
    end

    test "successful response of void with valid params", %{opts: opts} do
      use_cassette "sagepay/successful response of void with right params" do
        opts_authoriize = opts[:opts_authoriize] ++ [transaction_type: "Deferred"]
        {:ok, response} = SagePay.authorize(@amount, @card, opts_authoriize)
        assert {:ok, response} = SagePay.void(response.id, opts_authoriize)
    ...
    test "unsuccessful response of void with invalid params", %{opts: opts} do
      use_cassette "sagepay/unsuccessful response of void with invalid params" do
        opts_authoriize = opts[:opts_authoriize] ++ [transaction_type: "Deferred"]
        assert {:error, response} = SagePay.void(@payment_id, opts_authoriize)
      end
    end
  end
end
